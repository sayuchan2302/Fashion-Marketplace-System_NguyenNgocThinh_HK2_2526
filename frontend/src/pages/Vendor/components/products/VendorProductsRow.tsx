...
const VendorProductsRow = ({
  ...
  const isMuted = product.status === 'draft' || isBanned || product.status === 'review';

  return (
    <motion.div
      className={`admin-table-row vendor-products ${isMuted ? 'row-muted' : ''}`}
      role="row"
      initial={{ opacity: 0, y: 8 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.2, delay: Math.min(index * 0.025, 0.14) }}
      whileHover={{ y: -1 }}
      onClick={isBanned ? undefined : onOpenEdit}
      style={{ cursor: isBanned ? 'default' : 'pointer' }}
    >
      ...
      <div role="cell" className="admin-actions" onClick={(event) => event.stopPropagation()}>
        {isBanned ? (
          <button className="admin-icon-btn subtle" title="Xem lý do bị chặn" onClick={onOpenView}>
            <Eye size={16} />
          </button>
        ) : (
          <>
